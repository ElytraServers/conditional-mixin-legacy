...
public class SimpleRestrictionChecker implements RestrictionChecker {
    ...
    private AnnotationNode getRestrictionAnnotation(String className) throws ClassNotFoundException {
        ClassNode classNode;
        try {
            classNode = MixinService.getService()
                .getBytecodeProvider()
            ...
            switch (type) {
                case MOD:
                    ...
                    if (!VersionChecker.doesModVersionSatisfyPredicate(modId, versionPredicates)) {
                        results.add(
                            new Result(
                                false,
                                String.format(
                                    "mod %s@%s does not matches version predicates %s",
                                    modId,
                                    versionString,
                                    versionPredicates)));
                        continue;
                    ...
                    results.add(
                        new Result(
                            true,
                            String.format("conflicted/unsupported mod %s@%s found", modId, versionString)));
                    break;

                case MIXIN:
                    String requiredMixinClassName = Annotations.getValue(condition, "value");
                    Objects.requireNonNull(
                        requiredMixinClassName,
                        "field tester is required for condition type MIXIN, as the required mixin class name");
                    if (!this.checkRestriction(requiredMixinClassName)) {
                        results.add(
                            new Result(
                                false,
                                String.format("required mixin class %s disabled", requiredMixinClassName)));
                        continue;
                    }
                    results.add(
                        new Result(true, String.format("conflicted mixin class %s found", requiredMixinClassName)));
                    break;
                    ...
                    try {
                        ...
                        if (clazz.isInterface()) {
                            ConditionalMixinMod.LOGGER
                                .error("Tester class {} is a interface, but it should be a class", clazz.getName());
                            continue;
                        }
                        tester = (ConditionTester) clazz.getConstructor()
                            .newInstance();
                    } catch (Exception e) {
                        ConditionalMixinMod.LOGGER.error(
                            "Failed to instantiate a ConditionTester from class {}: {}",
                            clazzType.getClassName(),
                            e);
                        continue;
                    }
